<?php require_once __DIR__ . "/../layouts/header.php"; ?>

<div class="min-h-screen bg-gradient-to-br from-blue-100 via-white to-blue-200">
    <!-- Barra superior -->
    <header class="flex items-center justify-between px-10 py-5 bg-blue-800 shadow-lg">
        <div class="flex items-center gap-4">
            <svg class="w-10 h-10 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M12 11c0-1.104.896-2 2-2s2 .896 2 2-.896 2-2 2-2-.896-2-2zm0 0V7m0 4v4m0 0c-1.104 0-2 .896-2 2s.896 2 2 2 2-.896 2-2-.896-2-2-2z" />
            </svg>
            <span class="text-2xl font-bold text-white tracking-wide">Panel Administrador</span>
        </div>
        <nav class="flex gap-6">
            <a href="PROY-LPII/Vistas/Usuario/verUsuario.php"
                class="flex items-center gap-2 text-white hover:text-blue-200 transition font-medium">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M17 20h5v-2a4 4 0 00-3-3.87M9 20H4v-2a4 4 0 013-3.87M16 7a4 4 0 11-8 0 4 4 0 018 0z" />
                </svg>
                Usuarios
            </a>
            <a href="../vistas/Expediente/verExpediente.php"
                class="flex items-center gap-2 text-white hover:text-blue-200 transition font-medium">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M9 17v-2a2 2 0 012-2h2a2 2 0 012 2v2m-6 4h6a2 2 0 002-2v-5a2 2 0 00-2-2h-2a2 2 0 00-2 2v5a2 2 0 002 2z" />
                </svg>
                Expedientes
            </a>
            <a href="../vistas/Cita/verCita.php"
                class="flex items-center gap-2 text-white hover:text-blue-200 transition font-medium">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                </svg>
                Citas
            </a>
        </nav>
        <a href="/PROY-LPII/logout.php"
            class="flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white px-5 py-2 rounded-lg font-semibold shadow transition">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M17 16l4-4m0 0l-4-4m4 4H7" />
            </svg>
            Cerrar sesión
        </a>
    </header>

    <!-- Contenido principal -->
    <main class="max-w-6xl mx-auto py-12 px-4">
        <h1 class="text-4xl font-extrabold text-blue-800 mb-10 text-center flex items-center justify-center gap-3">
            <svg class="w-8 h-8 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M12 11c0-1.104.896-2 2-2s2 .896 2 2-.896 2-2 2-2-.896-2-2zm0 0V7m0 4v4m0 0c-1.104 0-2 .896-2 2s.896 2 2 2 2-.896 2-2-.896-2-2-2z" />
            </svg>
            Bienvenido Administrador
        </h1>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            <a href="/PROY-LPII/Vistas/Usuario/verUsuario.php"
                class="group bg-white shadow-lg p-8 rounded-xl hover:bg-blue-50 transition flex flex-col items-center text-center border border-transparent hover:border-blue-200">
                <div class="bg-blue-100 rounded-full p-4 mb-4 group-hover:bg-blue-200 transition">
                    <svg class="h-10 w-10 text-blue-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M17 20h5v-2a4 4 0 00-3-3.87M9 20H4v-2a4 4 0 013-3.87M16 7a4 4 0 11-8 0 4 4 0 018 0z" />
                    </svg>
                </div>
                <h2 class="text-xl font-bold text-blue-800 mb-1">Gestión de Usuarios</h2>
                <p class="text-gray-500">Crear, editar o eliminar usuarios.</p>
            </a>
            <a href="../vistas/Expediente/verExpediente.php"
                class="group bg-white shadow-lg p-8 rounded-xl hover:bg-blue-50 transition flex flex-col items-center text-center border border-transparent hover:border-blue-200">
                <div class="bg-blue-100 rounded-full p-4 mb-4 group-hover:bg-blue-200 transition">
                    <svg class="h-10 w-10 text-blue-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M9 17v-2a2 2 0 012-2h2a2 2 0 012 2v2m-6 4h6a2 2 0 002-2v-5a2 2 0 00-2-2h-2a2 2 0 00-2 2v5a2 2 0 002 2z" />
                    </svg>
                </div>
                <h2 class="text-xl font-bold text-blue-800 mb-1">Gestión de Expedientes</h2>
                <p class="text-gray-500">Asignar casos y ver estado de expedientes.</p>
            </a>
            <a href="../vistas/Cita/verCita.php"
                class="group bg-white shadow-lg p-8 rounded-xl hover:bg-blue-50 transition flex flex-col items-center text-center border border-transparent hover:border-blue-200">
                <div class="bg-blue-100 rounded-full p-4 mb-4 group-hover:bg-blue-200 transition">
                    <svg class="h-10 w-10 text-blue-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                    </svg>
                </div>
                <h2 class="text-xl font-bold text-blue-800 mb-1">Citas y Recordatorios</h2>
                <p class="text-gray-500">Programar citas y ver recordatorios.</p>
            </a>
        </div>
    </main>
</div>

<?php require_once __DIR__ . "/../layouts/footer.php"; ?>